<!--    设置ref名称，相当于js的id，能获取整个dom节点-->
<!--    单个小表单   label是名称，prop是定义校验的名称 @click是监听事件-->
<template>
  <div class="page">
    <PageHead/>
    <div class="center">
...
export default {
  ...
  methods: {
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          login(this.form).then(res => {
            this.$router.push({name:'student_index',params:{id:this.form.user_no}})
            if (res.code === 0) {
    ...
    goToRegister () {
      this.$router.push('/student_register')
    },
    goToChange () {
